#pragma once

/*
* SDK generated by Dumper-7
*
* https://github.com/Encryqed/Dumper-7
*/

// Package: ImgMedia

#include "Basic.hpp"

#include "CoreUObject_structs.hpp"
#include "Engine_structs.hpp"
#include "MediaAssets_classes.hpp"


namespace SDK
{

// Class ImgMedia.ImgMediaSource
// 0x0028 (0x00B0 - 0x0088)
class UImgMediaSource final : public UBaseMediaSource
{
public:
	struct FFrameRate                             FrameRateOverride;                                 // 0x0088(0x0008)(Edit, BlueprintVisible, ZeroConstructor, IsPlainOldData, NoDestructor, AdvancedDisplay, HasGetValueTypeHash, NativeAccessSpecifierPublic)
	class FString                                 ProxyOverride;                                     // 0x0090(0x0010)(Edit, BlueprintVisible, ZeroConstructor, AdvancedDisplay, HasGetValueTypeHash, NativeAccessSpecifierPublic)
	struct FDirectoryPath                         SequencePath;                                      // 0x00A0(0x0010)(Edit, BlueprintVisible, BlueprintReadOnly, Protected, NativeAccessSpecifierProtected)

public:
	void SetSequencePath(const class FString& Path); // (Final, Native, Public, BlueprintCallable) // @ 0x1428F9FB0

	void GetProxies(TArray<class FString>* OutProxies) const; // (Final, Native, Public, HasOutParams, BlueprintCallable, BlueprintPure, Const) // @ 0x1428F9DB0
	const class FString GetSequencePath() const; // (Final, Native, Public, BlueprintCallable, BlueprintPure, Const) // @ 0x1428F9ED0

public:
	static class UClass* StaticClass()
	{
		STATIC_CLASS_IMPL("ImgMediaSource")
	}
	static const class FName& StaticName()
	{
		STATIC_NAME_IMPL(L"ImgMediaSource")
	}
	static class UImgMediaSource* GetDefaultObj()
	{
		return GetDefaultObjImpl<UImgMediaSource>();
	}
};
DUMPER7_ASSERTS_UImgMediaSource;

}

